...
learning_rates = [5e-5] # potentially 3e-5, but picking 5e-5 since will take less time
#learning_rates = [4e-5]
...
"""
def preprocess_dataset(data_subset, max_input_length=max_input_length):
    print("\nStarting preprocessing...")
    print(f"\nToken length: {max_input_length}")
    data_subset_size = len(data_subset)
    processed_data = []
...
def train_model(selected_learning_rate):
    ...
    main_dir = rf"E:/NLPProject/experiments/bill_sum/{current_model}_il{str(max_input_length)}_bs{str(batch_size)}_as{accumulation_steps}_ol{str(generation_max_length)}_ts{training_samples}/{str(selected_learning_rate)}"
...
data_train = dataset["train"].select(range(training_samples))
data_validation = dataset["train"].select(range(training_samples, training_samples + 500))

# Preprocessed dataset splits and conversion from list type to dataset object.
processed_train_data = preprocess_dataset(data_train)
processed_val_data = preprocess_dataset(data_validation)
